...
/** Generates name-based starter rules from unambiguous inventory label mappings. */
public final class StarterRuleGenerator {
    ...
    public RuleSet generate(
            Inventory inventory, Map<String, String> userLabelNamesById, int minimumMessageCount) {
        ...
        inventory.domains().stream()
                .map(domain -> domainRule(domain, userLabelNamesById, minimumMessageCount))
                .flatMap(Optional::stream)
                .forEach(rules::add);
        inventory.senders().stream()
                .map(sender -> senderRule(sender, userLabelNamesById, minimumMessageCount))
                .flatMap(Optional::stream)
                .forEach(rules::add);
    ...
    private static Optional<RuleDefinition> senderRule(
            SenderInventory sender, Map<String, String> labels, int minimumCount) {
        return mappedLabelName(sender.statistics(), labels, minimumCount)
                .map(label -> rule(
    ...
    private static Optional<RuleDefinition> domainRule(
            DomainInventory domain, Map<String, String> labels, int minimumCount) {
        return mappedLabelName(domain.statistics(), labels, minimumCount)
                .map(label -> rule(
    ...
    private static Optional<String> mappedLabelName(
        ...
        List<String> matchingLabels = statistics.currentLabels().stream()
                .filter(userLabelNamesById::containsKey)
                .distinct()
                .toList();
        return matchingLabels.size() == 1
                ? Optional.of(userLabelNamesById.get(matchingLabels.getFirst()))
                : Optional.empty();
    }
